<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="doList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="操作id" align="center" prop="id" />
      <el-table-column label="操作人" align="center" prop="doUser" />
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="操作人" prop="doUser">
          <el-input v-model="form.doUser" placeholder="请输入操作人" />
        </el-form-item>
        <el-form-item label="操作时间" prop="doTime">
          <el-date-picker clearable size="small"
            v-model="form.doTime"
        ...
        <el-form-item label="操作对象id" prop="doId">
          <el-input v-model="form.doId" placeholder="请输入操作对象id" />
        </el-form-item>
        <el-form-item label="操作对象类型" prop="doType">
          <el-select v-model="form.doType" placeholder="请选择操作对象类型">
            <el-option label="请选择字典生成" value="" />
          </el-select>
        </el-form-item>
      </el-form>
...
<script>
import { listDo, getDo, delDo, addDo, updateDo, exportDo } from "@/api/medical/do";
...
export default {
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$confirm('是否确认删除操作记录编号为"' + ids + '"的数据项?', "警告", {
    ...
    handleExport() {
      ...
      this.$confirm('是否确认导出所有操作记录数据项?', "警告", {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
          type: "warning"
        }).then(() => {
          this.exportLoading = true;
          return exportDo(queryParams);
        }).then(response => {
