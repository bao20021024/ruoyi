<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="resultList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="医生看病结果id" align="center" prop="opDoctorResultId" />
      <el-table-column label="病名称" align="center" prop="name" />
      <el-table-column label="描述症状" align="center" prop="description" />
      <el-table-column label="医嘱" align="center" prop="remark" />
      <el-table-column label="门诊医生接诊记录id" align="center" prop="opDoctorReceiveRecordId" />
...
<script>
import { listResult, getResult, delResult, addResult, updateResult, exportResult } from "@/api/medical/result";
...
export default {
  ...
  methods: {
    ...
    handleSelectionChange(selection) {
      this.ids = selection.map(item => item.opDoctorResultId)
    ...
    handleUpdate(row) {
      this.reset();
      const opDoctorResultId = row.opDoctorResultId || this.ids
      getResult(opDoctorResultId).then(response => {
    ...
    submitForm() {
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.opDoctorResultId != null) {
    ...
    /** 删除按钮操作 */
    handleDelete(row) {
      const opDoctorResultIds = row.opDoctorResultId || this.ids;
      this.$confirm('是否确认删除医生看病结果编号为"' + opDoctorResultIds + '"的数据项?', "警告", {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
          type: "warning"
        }).then(function() {
          return delResult(opDoctorResultIds);
        }).then(() => {
          this.getList();
    ...
    handleExport() {
      ...
      this.$confirm('是否确认导出所有医生看病结果数据项?', "警告", {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
          type: "warning"
        }).then(() => {
