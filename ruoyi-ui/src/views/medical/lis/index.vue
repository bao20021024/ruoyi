...
<script>
import { listLis, getLis, delLis, addLis, updateLis, exportLis } from "@/api/medical/lis";
...
export default {
  ...
  methods: {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            ...
            addLis(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$confirm('是否确认删除Lis检验信息编号为"' + ids + '"的数据项?', "警告", {
    ...
    handleExport() {
      ...
      this.$confirm('是否确认导出所有Lis检验信息数据项?', "警告", {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
          type: "warning"
        }).then(() => {
          this.exportLoading = true;
          return exportLis(queryParams);
        }).then(response => {
